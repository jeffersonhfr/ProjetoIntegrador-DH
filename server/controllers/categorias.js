...
const controller = {
  index: async (req, res, next) => {
    try {
      ...
      if (!categorias)
        return res
          .status(200)
          .send({ message: 'Não achamos nenhuma categoria' });
  ...
  create: async (req, res, next) => {
    try {
      ...
      if (!nomeCategoria)
        return res
          .status(400)
          .send({ message: 'Campo Nome da categoria vazio' });
      if (!corCategoria)
        return res
          .status(400)
          .send({ message: 'Campo Cor da categoria vazio' });
      if (!req.file)
        return res
          .status(400)
          .send({ message: 'A categoria precisa de uma imagem' });
      var imagemCategoria = `${req.file.filename}`;

      const create = await createCategoria({
        nomeCategoria,
        corCategoria,
        imagemCategoria,
      });
  ...
  update: async (req, res, next) => {
    try {
      ...
      var imagemCategoria;
      if (req.file) imagemCategoria = `${req.file.filename}`;
      ...
      if (!imagemCategoria)
        ...
        update = await updateCategoria(id, {
          nomeCategoria,
          corCategoria,
          imagemCategoria,
        });
